...
def setup_platform(hass, config, add_devices, discovery_info=None):
    """Setup the Polyhome LightPanel platform."""
    ...
    def event_zigbee_recv_handler(event):
        ...
        if bytearr[0] == '0xa0' and bytearr[8] == '0x73' and bytearr[9] == '0x24':
        ...
        if bytearr[0] == '0xa0' and bytearr[5] == '0x24' and bytearr[8] == '0x79':
            """panel trigger"""
            mac_l, mac_h = bytearr[6].replace('0x',''), bytearr[7].replace('0x','')
            mac_str = mac_l + '#' + mac_h
            dev = next((dev for dev in lights if dev.mac == mac_str), None)
...
class PolyLightPanel(Light):
    """Representation of an Polyhome LightPanel Class."""
    ...
    def device_state_attributes(self):
        ...
        """
        return {'platform': 'polylightpanel'}
