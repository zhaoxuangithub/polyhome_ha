...
def setup_platform(hass, config, add_devices, discovery_info=None):
    ...
    if discovery_info is not None:
        ...
        panels.append(PolyZfirePanel3(hass, device, None))
        device = {'name': discovery_info['name'] +
                  '2', 'mac': discovery_info['mac'], 'way': 2}
        panels.append(PolyZfirePanel3(hass, device, None))
        device = {'name': discovery_info['name'] +
                  '3', 'mac': discovery_info['mac'], 'way': 3}
        panels.append(PolyZfirePanel3(hass, device, None))
    else:
        for mac, device_config in config['devices'].items():
            device = {
                'name': device_config['name'] + '1', 'mac': mac, 'way': 1}
            panels.append(PolyZfirePanel3(hass, device, device_config))
            device = {
                'name': device_config['name'] + '2', 'mac': mac, 'way': 2}
            panels.append(PolyZfirePanel3(hass, device, device_config))
            device = {
                'name': device_config['name'] + '3', 'mac': mac, 'way': 3}
            panels.append(PolyZfirePanel3(hass, device, device_config))
    ...
    def event_zigbee_recv_handler(event):
        ...
        if pack_list[0] == '0xa0' and pack_list[5] == '0x3E':
            ...
            if dev is not None:
                ...
                if pack_list[9] == '0x2':
                    dev.trigger('2')
        if pack_list[0] == '0xa0' and pack_list[5] == '0x3e' and pack_list[8] == '0x70':
            ...
            for dev in panels:
                if mac_str in dev.mac:
                    ...
                    if dev.way == 3 and pack_list[11] == '0x0':
                        dev.set_state(False)
        if pack_list[0] == '0xa0' and pack_list[5] == '0x3e' and pack_list[8] == '0xcc':
            ...
            for dev in panels:
                if mac_str in dev.mac:
                    ...
                    if dev.way == 3 and pack_list[11] == '0x0':
                        dev.set_state(False)
        if pack_list[0] == '0xa0' and pack_list[5] == '0x3e' and pack_list[8] == '0x77':
            # device status
            mac_l, mac_h = pack_list[6].replace('0x', ''), pack_list[7].replace('0x', '')
            mac_str = mac_l + '#' + mac_h
            for dev in panels:
...
class PolyZfirePanel3(Light):
    """Representation of an Polyhome ZfirePanel3 Class."""

    def __init__(self, hass, device, dev_conf):
        """Initialize an PolyZfirePanel3."""
        self._hass = hass
        self._device = device
    ...
    @property
    def way(self):
        """Return the display way of this light."""
        return self._way
    ...
    def device_state_attributes(self):
        ...
        """
        return {'platform': 'polyzfirepanel3'}
