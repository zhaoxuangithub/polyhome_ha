...
def setup_platform(hass, config, add_devices, discovery_info=None):
                ...
                for state in hass.states.async_all():
                    ...
                    if entity_id == state_dict['entity_id']:
                        ...
                        device = {'name': discovery_info['name'] + str(index), 'mac': discovery_info['mac'],
                                  'index': index}
                        climates.append(PolyIraccClimate(hass, device, None))
                if not b_find:
                    device = {'name': discovery_info['name'] + str(index), 'mac': discovery_info['mac'],
                              'index': index}
                    climates_new.append(PolyIraccClimate(hass, device, None))
    ...
    def service_climate_adapter_change_handler(service):
        state = service.data.get('iracc').get('state')
        mac = service.data.get('iracc').get('mac')
        for climate in climates:
            if climate.mac == mac:
                climate.set_state(state)
    hass.services.register('iracc', 'iracc_adapter_state_change', service_climate_adapter_change_handler)
...
class PolyIraccClimate(ClimateDevice):
    """Polyhome IraccClimate."""
    ...
    def device_state_attributes(self):
        ...
        """
        return {'platform': 'polyiracc', 'run_state': self._on, 'operation_mode': self._operation_mode, \
