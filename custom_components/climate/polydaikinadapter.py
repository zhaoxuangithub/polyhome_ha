...
def setup_platform(hass, config, add_devices, discovery_info=None):
    """Set up the polydaikinadapter platform."""
    ...
    if discovery_info is not None:
        ...
        climates.append(PolyDaiKinAdapter(hass, device, config))
    else:
        ...
        if time_init == 14:
            for device in climates:
                if device.state:
                    ...
                    print('%s DaiKin Adapter is OffLine' %(device.mac))
            time_init = 0
...
"""
   大金空调管理者
"""
class PolyDaiKinAdapter(Entity):
    """Representation of a DaiKinAdapter."""
    ...
    def device_state_attributes(self):
        ...
        return {'platform': 'polydaikinadapter'}
    ...
    def set_state(self, state):
        ...
        for states in self._hass.states.async_all():
            ...
            if entity_id == state_dict['entity_id']:
                b_find = True
        if b_find:
            self._hass.services.call('daikin', 'daikin_adapter_state_change', \
                                     {'daikin': {'mac': self._mac, 'state': self._state}})
